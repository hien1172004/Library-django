...
class AddBookView(generics.CreateAPIView):  # checked
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Failed to add book", "errors": serializer.errors},
...
class DeleteBookView(generics.DestroyAPIView):  # checked
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not book_id:
            return Response(
                {"message": "'Book id is required."}, status=status.HTTP_400_BAD_REQUEST
            )
            ...
            return Response(
                {
                    ...
                    "refresh": str(
                        refresh
                    ),  # Trả về token làm mới (refresh token) - tùy chọn nhưng khuyến khích
                },
                status=status.HTTP_200_OK,
            )  # Trả về mã trạng thái 200 (thành công)
        else:
            return Response(
...
class ChangePasswordView(APIView):  # checked
    permission_classes = [
        IsAuthenticated
    ]  # Chỉ người dùng đã đăng nhập mới có quyền thay đổi mật khẩu
...
class StudentAddView(generics.CreateAPIView):  # checked
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializers.is_valid():
            serializers.save()
            return Response(
                {"message": "Student added sucessfully!", "data": serializers.data},
                status=status.HTTP_201_CREATED,
            )
        return Response(
            {"message": "Failed to add student", "errors": serializers.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
class StudentEditView(generics.UpdateAPIView):  # checked
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializers.is_valid():
            serializers.save()
            return Response(
                {"message": "student update Success", "data": serializers.data},
                status=status.HTTP_200_OK,
            )
        return Response(
            {"errors": "Failed to update student", "data": serializers.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
class CheckInView(generics.CreateAPIView):  # checked
    ...
    def create(self, request, *args, **kwargs):
        ...
        existing_log = LibraryLog.objects.filter(
            student_id=student_id, checked_out__isnull=True
        ).first()
        if existing_log:
            return Response(
                {
        ...
        return Response(
            {"message": "Check-in successful", "data": serializer.data},
            status=status.HTTP_201_CREATED,
        )
...
class CheckOutView(generics.UpdateAPIView):  # checked
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Check-out successful", "data": serializer.data},
            status=status.HTTP_200_OK,
        )
...
class LibraryLogCountInDayView(APIView):
    def post(self, request):
        ...
        if not count or not isinstance(count, int) or count <= 0:
            return Response(
                {"error": "Tham số count không hợp lệ."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for day in all_days:
            ...
            day_end = int(
                time.mktime((day + datetime.timedelta(days=1)).timetuple())
            )  # Tính đến cuối ngày

            # Truy vấn và đếm số lượt check-in trong ngày
            day_logs = LibraryLog.objects.filter(
                checked_in__gte=day_start, checked_in__lt=day_end
...
class BookTransactionAddView(generics.CreateAPIView):  # checked
    ...
    def create(self, request, *args, **kwargs):
        ...
        if book.quantity <= 0:
            return Response(
                {"message": "Book not available"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class BookTransactionSearchView(generics.ListAPIView):  # checked
    ...
    def get_queryset(self):
        ...
        if student_id:
            filters |= Q(
                student__student_id__icontains=student_id
            )  # Case-insensitive partial match for student_id
        if book_id:
            ...
            for i in queryset:
                ...
                if remain_day >= int(day_remaining):
                    filtered_queryset.append(
                        i
                    )  # Thêm đối tượng vào danh sách nếu thỏa mãn điều kiện
        ...
        if order == "ASC":
            ...
            queryset = sorted(
                queryset, key=lambda x: getattr(x, order_by), reverse=True
            )
        return queryset
...
class BookTransactionReturnView(generics.UpdateAPIView):  # checked
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not student_id or not book_id:
            return Response(
                {"error": "Both student_id and book_id are required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {
                    "error": "Transaction not found or the book has already been returned."
                },
        ...
        if not transaction.return_date:
            transaction.return_date = int(
                time.time()
            )  # Đặt ngày trả thành ngày hiện tại
            transaction.save()
            # Tăng số lượng sách
            ...
            serializer = self.get_serializer(transaction)
            return Response(
                {"message": "Book successfully returned!", "data": serializer.data},
                status=status.HTTP_200_OK,
            )

        return Response(
            {"error": "Transaction is already marked as returned"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
class CategoryAddView(generics.CreateAPIView):  # checked
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializers.is_valid():
            serializers.save()
            return Response(
                {"message": "Category added sucessfully!", "data": serializers.data},
                status=status.HTTP_201_CREATED,
            )
        return Response(
            {"message": "Failed to add Category", "errors": serializers.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
class CategoryEditview(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializers.is_valid():
            serializers.save()
            return Response(
                {"message": "category update Success", "data": serializers.data},
                status=status.HTTP_200_OK,
            )
        return Response(
            {"errors": "Failed to update category_id", "data": serializers.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
